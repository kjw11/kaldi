...
if [ "$use_existing_models" == "true" ]; then
  for f in $plda_ivec_dir/mean.vec $plda_ivec_dir/lda_plda ; do
  ...
  if [ ! -f $plda_ivec_dir/transform_lda.mat ]; then
    run.pl $plda_ivec_dir/log/lda.log \
      ...
      ark:$plda_data_dir/utt2spk $plda_ivec_dir/transform_lda.mat || exit 1;
  fi

  if [ ! -f $plda_ivec_dir/lda_plda ]; then
    run.pl $plda_ivec_dir/log/lda_plda.log \
      ivector-compute-plda ark:$plda_data_dir/spk2utt \
      "ark:ivector-subtract-global-mean scp:$plda_ivec_dir/ivector.scp ark:- | transform-vec $plda_ivec_dir/transform_lda.mat ark:- ark:- | ivector-normalize-length ark:- ark:- |" \
      $plda_ivec_dir/lda_plda || exit 1;
  fi
fi
...
run.pl $scores_dir/log/lda_plda_scoring.log \
  ivector-plda-scoring --normalize-length=true \
    ...
    "ivector-copy-plda --smoothing=0.0 ${plda_ivec_dir}/lda_plda - |" \
    "ark:ivector-mean ark:$enroll_data_dir/spk2utt scp:$enroll_ivec_dir/ivector.scp ark:- | ivector-subtract-global-mean $plda_ivec_dir/mean.vec ark:- ark:- | transform-vec $plda_ivec_dir/transform_lda.mat ark:- ark:- | ivector-normalize-length ark:- ark:- |" \
    "ark:ivector-subtract-global-mean $plda_ivec_dir/mean.vec scp:$test_ivec_dir/ivector.scp ark:- | transform-vec $plda_ivec_dir/transform_lda.mat ark:- ark:- | ivector-normalize-length ark:- ark:- |" \
