...
if [ "$use_existing_models" == "true" ]; then
  for f in ${plda_xvec_dir}/mean.vec ${plda_xvec_dir}/plda ; do
    [ ! -f $f ] && echo "No such file $f" && exit 1;
  done
else
  run.pl $plda_xvec_dir/log/compute_mean.log \
    ivector-normalize-length scp:${plda_xvec_dir}/xvector.scp \
    ark:- \| ivector-mean ark:- ${plda_xvec_dir}/mean.vec || exit 1;

  run.pl $plda_xvec_dir/log/plda.log \
    ivector-compute-plda ark:$plda_data_dir/spk2utt \
    "ark:ivector-normalize-length scp:${plda_xvec_dir}/xvector.scp  ark:- |" \
    $plda_xvec_dir/plda || exit 1;
fi
...
run.pl $scores_dir/log/plda_scoring.log \
  ivector-plda-scoring --normalize-length=true \
    ...
    "ivector-copy-plda --smoothing=0.0 ${plda_xvec_dir}/plda - |" \
    "ark:ivector-subtract-global-mean ${plda_xvec_dir}/mean.vec scp:${enroll_xvec_dir}/spk_xvector.scp ark:- | ivector-normalize-length ark:- ark:- |" \
    "ark:ivector-normalize-length scp:${test_xvec_dir}/xvector.scp ark:- | ivector-subtract-global-mean ${plda_xvec_dir}/mean.vec ark:- ark:- | ivector-normalize-length ark:- ark:- |" \
